...
def visualize(img_arr, dpi):
    ...
    plt.imshow(
        ((img_arr.detach().cpu().numpy().transpose(1, 2, 0) + 1.0) * 127.5).astype(
            np.uint8
        )
    )
    plt.axis("off")
...
def save_image(img, filename):
    tmp = ((img.detach().cpu().numpy().transpose(1, 2, 0) + 1.0) * 127.5).astype(
        np.uint8
    )
...
def gather_params(G):
    params = dict([(res, {}) for res in range(18)] + [("others", {})])
...
def blend_models(G_low, G_high, weight=[1] * 7 + [0] * 11):
    ...
    for res in range(18):
        for n, p in params_high[res].items():
            params_high[res][n] = (
                params_high[res][n] * (1 - weight[res])
                + params_low[res][n] * weight[res]
            )
