    ...
    for _filename in filelist:
        ...
        if args.video:
            ...
            savename = os.path.join(
                args.output_path, basename + "_vtoonify_" + args.backbone[0] + ".mp4"
            )
                ...
                if first_valid_frame:
                    if args.scale_image:
                        paras = get_video_crop_parameter(
                            frame, landmarkpredictor, args.padding
                        )
                        if paras is None:
                            continue
                    ...
                    videoWriter = cv2.VideoWriter(
                        cropname, fourcc, video_cap.get(5), (W, H)
                    )
                    videoWriter2 = cv2.VideoWriter(
                        savename, fourcc, video_cap.get(5), (4 * W, 4 * H)
                    )
